...
func getEndpointSelector(namespace string, labelSelector *slim_metav1.LabelSelector, addK8sPrefix, matchesInit bool) api.EndpointSelector {
	...
	if !es.HasKey(podPrefixLbl) && !es.HasKey(podAnyPrefixLbl) && !es.HasKey(podOpenstackPrefixLbl) && !es.HasKey(podCalicoPrefixLbl) {
		if namespace == "" {
...
func ParseToCiliumRule(namespace, name string, uid types.UID, r *api.Rule) *api.Rule {
	...
	if r.EndpointSelector.LabelSelector != nil {
		...
		if namespace != "" {
			...
			userNamespace, present := r.EndpointSelector.GetMatch(prefix)
			...
			retRule.EndpointSelector.AddMatch(prefix, namespace)
		}
